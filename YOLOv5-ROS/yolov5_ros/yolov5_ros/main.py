...
while (current_folder != 'install' or i >= 6):
    i += 1
    current_folder = str(FILE.parents[i]).split("/")[-1]  # YOLOv5 root folder, needed for imports to work


sys.path.insert(0, os.path.join(FILE.parents[i], 'share', 'yolov5_ros'))
sys.path.insert(0, os.path.join(FILE.parents[i], 'yolov5_ros', 'share', 'yolov5_ros'))
standard_location = i

#pdb.set_trace()


#sys.path.insert(0, os.path.join(sys.path[0], '..', '..', '..', '..', 'src', 'perception', 'YOLOv5-ROS', 'yolov5_ros')) #WARNING, IF THE YOLOV5 SCRIPTS ARE CHANGED OF
# FOLDER, THE IMPORTS BELOW WON'T WORK!
...
class yolov5_demo():
    ...
    def image_callback(self, image_raw):
        ...
        self.img_size = 640
        #image_raw = self.br.compressed_imgmsg_to_cv2(image_raw)
...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.sub_image = self.create_subscription(CompressedImage, '/f_camera2', self.image_callback,10)
        ...
        trained_weights = '27SEP_Best.pt'  # '27SEP_Best.pt' 'yolov5s.pt'
        self.declare_parameter('weights', os.path.join(FILE.parents[standard_location], 'share', 'yolov5_ros', 'config', trained_weights))
        self.declare_parameter('data', os.path.join(FILE.parents[standard_location], 'share', 'yolov5_ros', 'data', 'coco128.yaml'))
    ...
    def image_callback(self, image:CompressedImage):
        image_raw = self.bridge.compressed_imgmsg_to_cv2(image)
        # return (class_list, confidence_list, x_min_list, y_min_list, x_max_list, y_max_list)
        ...
        image_message = self.bridge.cv2_to_imgmsg(image_raw, encoding="bgr8")
